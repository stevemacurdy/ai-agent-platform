#!/bin/bash
# ============================================================
# WoulfAI P8: FINAL SWEEP — All remaining pages
# Run from repo root: bash scripts/transform-final.sh
#
# Catches:
#   - billing, careers, dashboard, demo/[slug], invite, 
#     portal/agent/*, s/[slug], sales/settings
#   - Residual dark patterns in P0-P2 pages
#   - warehouse/layout.tsx residuals
# ============================================================

echo ""
echo "🎨 WoulfAI P8: Final Sweep"
echo "══════════════════════════════════════"

# Collect ALL non-API page and layout files
FILES=$(find app/ -name "page.tsx" -o -name "layout.tsx" | grep -v "/api/" | sort)
...
for FILE in $FILES; do
  ...
  # Check if file has any dark patterns before processing
  if ! grep -qE "bg-\[#0|bg-white/[0-9]|border-white/|text-gray-[3-6]00|text-white/[2-9]|hover:bg-white/|hover:text-white|placeholder-gray|placeholder-white|focus:border-blue|focus:ring-blue|bg-gray-[89]|border-gray-[78]|divide-gray-8|divide-white|ring-white|text-blue-[34]00|text-green-400|text-emerald-400|text-amber-400|text-purple-400|text-red-400|Agent" "$FILE" 2>/dev/null; then
    continue
  fi
  ...
  # ── Agent → Employee (generic, safe patterns) ──
...
echo ""
echo "══════════════════════════════════════"
echo "✅ Final sweep: $CHANGED files processed"
echo ""
echo "Run:"
echo "  npm run build"
echo "  git add -A"
echo "  git commit -m 'P8: Final sweep - remaining dark patterns + Agent language'"
echo "  vercel --prod"
echo ""
