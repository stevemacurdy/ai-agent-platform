...
export default function FinOpsProPage() {
  ...
  const loadTax = async () => { const r = await fetch('/api/tax-reserve', { headers: { 'x-admin-email': getEmail() } }); setTaxData(await r.json()) }

  // ====== DUPLICATES ======
  const [dupData, setDupData] = useState<any>(null)
  const loadDups = async () => { const r = await fetch('/api/duplicate-detection', { headers: { 'x-admin-email': getEmail() } }); setDupData(await r.json()) }
  ...
  const loadAnom = async () => { const r = await fetch('/api/anomaly', { headers: { 'x-admin-email': getEmail() } }); setAnomData(await r.json()) }

  // ====== VENDORS ======
  const [vendorData, setVendorData] = useState<any>(null)
  const loadVendors = async () => { const r = await fetch('/api/vendor-scoring', { headers: { 'x-admin-email': getEmail() } }); setVendorData(await r.json()) }

  // ====== LENDING ======
  const [lendingData, setLendingData] = useState<any>(null)
  const [lendingLoading, setLendingLoading] = useState(false)
  const loadLending = async () => { const r = await fetch('/api/lending-packet?view=preview', { headers: { 'x-admin-email': getEmail() } }); setLendingData(await r.json()) }
