...
export default function CFOConsoleV2() {
  ...
  const ST = { paid: 'bg-emerald-50 text-emerald-600', overdue: 'bg-rose-500/10 text-rose-400', partial: 'bg-amber-50 text-amber-600', sent: 'bg-blue-50 text-blue-600' }
  ...
  const HC = { good: 'text-emerald-600', warning: 'text-amber-600', critical: 'text-rose-400', info: 'text-[#4B5563]' }
          ...
          <div key={i} className="flex-1 flex flex-col items-center gap-1">
            <div className="flex items-end gap-1 h-36 w-full justify-center">
              <div className="flex flex-col items-center w-8">
                <div className="text-[8px] text-emerald-600 font-mono mb-0.5">{fmt(w.inflows.total)}</div>
              ...
              <div className="flex flex-col items-center w-8">
                <div className={'text-[8px] font-mono mb-0.5 ' + (w.netCash >= 0 ? 'text-blue-600' : 'text-rose-400')}>{fmt(w.netCash)}</div>
            ...
            <div className="text-[9px] text-[#6B7280]">Cash: {fmt(w.endingCash)}</div>
          </div>
        ))}
      </div>
    )
  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-bold">CFO Intelligence Console</h1><p className="text-sm text-[#9CA3AF] mt-1">Click any number for source data. Click any button to trigger analysis.</p></div>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {[
            { label: 'Total AR', value: summary.totalAR, color: 'text-white', border: 'border-[#E5E7EB]', filter: () => true },
            { label: 'Overdue', value: summary.overdueTotal, color: 'text-rose-400', border: 'border-rose-500/20', filter: (i: any) => i.status === 'overdue' },
            { label: 'Collected', value: summary.paidTotal, color: 'text-emerald-600', border: 'border-emerald-500/20', filter: (i: any) => i.status === 'paid' },
            { label: 'Partial', value: invoices.filter(i => i.status === 'partial').reduce((s, i) => s + i.amount - i.amountPaid, 0), color: 'text-amber-600', border: 'border-amber-500/20', filter: (i: any) => i.status === 'partial' },
      ...
      <div className="grid grid-cols-4 gap-3">
        <button onClick={runCashflow} className="bg-gradient-to-br from-blue-500/5 to-blue-500/10 border border-blue-500/20 rounded-xl p-5 text-left hover:border-blue-500/40 transition-all">
          ...
          <div className="text-[10px] text-[#9CA3AF] mt-1">30/60/90-day with Odoo + HubSpot data</div>
        </button>
        <button onClick={runCollections} className="bg-gradient-to-br from-rose-500/5 to-rose-500/10 border border-rose-500/20 rounded-xl p-5 text-left hover:border-rose-500/40 transition-all">
          ...
          <div className="text-[10px] text-[#9CA3AF] mt-1">Prioritized debtor queue + draft messages</div>
        </button>
      ...
      {modal === 'invoice' && selectedInv && (
        <Modal>
          <div className="flex items-start justify-between mb-5">
            <div>
              ...
              <div className="flex gap-3 mt-2 text-xs">
                ...
                <span className="text-[#9CA3AF]">Odoo: <span className="font-mono text-[#6B7280]">{selectedInv.odooId}</span></span>
              </div>
            </div>
          ...
          {invAudit.length > 0 && (
            <div>
              <h4 className="text-xs font-semibold uppercase text-[#9CA3AF] mb-2">Audit Trail</h4>
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {invAudit.map((a: any) => (
                  <div key={a.id} className="flex items-center justify-between text-[10px] py-1 border-b border-white/[0.02]">
                    <div className="flex gap-2"><span className="bg-white shadow-sm px-1.5 py-0.5 rounded text-[#6B7280] font-mono">{a.action}</span><span className="text-[#9CA3AF]">{a.userId}</span></div>
                    <div className="flex gap-2 text-[#6B7280]">
                      ...
                      {a.after && <span className="text-[#6B7280]">New: {typeof a.after === 'object' ? Object.entries(a.after).map(([k,v]) => `${k}:${v}`).join(', ') : String(a.after)}</span>}
          ...
          {cashflowLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-4 gap-3 mb-5">
                <div className="bg-white shadow-sm rounded-lg p-3"><div className="text-[9px] text-[#9CA3AF]">Starting Cash</div><div className="font-mono font-bold mt-1">{fmt(cashflow.cashOnHand)}</div></div>
                <div className="bg-white shadow-sm rounded-lg p-3"><div className="text-[9px] text-[#9CA3AF]">Total Inflows</div><div className="font-mono font-bold text-emerald-600 mt-1">{fmt(cashflow.totals?.totalInflow)}</div></div>
                <div className="bg-white shadow-sm rounded-lg p-3"><div className="text-[9px] text-[#9CA3AF]">Total Outflows</div><div className="font-mono font-bold text-rose-400 mt-1">{fmt(cashflow.totals?.totalOutflow)}</div></div>
                <div className="bg-white shadow-sm rounded-lg p-3"><div className="text-[9px] text-[#9CA3AF]">90-Day Cash</div><div className={'font-mono font-bold mt-1 ' + (cashflow.totals?.endingCash90 >= 0 ? 'text-emerald-600' : 'text-rose-400')}>{fmt(cashflow.totals?.endingCash90)}</div></div>
              </div>
                ...
                <div key={w.window} className="mb-3">
                  <button onClick={() => setExpandedWindow(expandedWindow === w.window ? null : w.window)}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[10px] text-emerald-600 font-mono">In: {fmt(w.inflows.total)}</span>
                      <span className="text-[10px] text-rose-400 font-mono">Out: {fmt(w.outflows.total)}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={'text-sm font-mono font-bold ' + (w.netCash >= 0 ? 'text-emerald-600' : 'text-rose-400')}>Net: {fmt(w.netCash)}</span>
                      <span className="text-[#6B7280]">{expandedWindow === w.window ? '\u25B2' : '\u25BC'}</span>
                    </div>
                  </button>
                  {expandedWindow === w.window && (
                    <div className="grid grid-cols-2 gap-3 mt-2 px-2">
                      <div>
                        <div className="text-[9px] text-emerald-600 uppercase mb-1">Inflows</div>
                        {w.inflows.arReceivables.items.map((r: any, i: number) => (
                          <div key={i} className="flex justify-between text-[10px] py-0.5 text-[#4B5563]">
                            <span className="font-mono text-blue-600">{r.number}</span><span>{r.partner}</span><span className="font-mono">{fmt(r.amount)}</span>
                          </div>
                        ))}
                        {w.inflows.hubspotDeals.items.map((d: any, i: number) => (
                          <div key={i} className="flex justify-between text-[10px] py-0.5 text-[#6B7280]">
                            <span className="text-violet-600">{d.stage}</span><span className="truncate max-w-[120px]">{d.name}</span><span className="font-mono">{fmt(d.weightedValue)} <span className="text-[#6B7280]">({Math.round(d.probability * 100)}%)</span></span>
                          </div>
                        ))}
                        {w.inflows.arReceivables.items.length === 0 && w.inflows.hubspotDeals.items.length === 0 && <div className="text-[10px] text-[#6B7280]">None</div>}
                      </div>
                      <div>
                        <div className="text-[9px] text-rose-400 uppercase mb-1">Outflows</div>
                        {w.outflows.vendorBills.items.map((b: any, i: number) => (
                          <div key={i} className="flex justify-between text-[10px] py-0.5 text-[#4B5563]">
                            <span>{b.vendor}</span><span className="font-mono">{fmt(b.amount)}{b.recurring && <span className="text-[#6B7280] ml-1">(mo)</span>}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div key={s.invoice.id} className={'border rounded-xl p-4 mb-3 ' + ((URG as any)[s.strategy.urgency] || '')}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-mono text-blue-600 cursor-pointer hover:underline" onClick={() => { setModal(null); setTimeout(() => openInvoice(s.invoice.id), 100) }}>{s.invoice.number}</span>
                      <span className="text-sm">{s.invoice.client}</span>
                      <span className="text-[10px] text-[#9CA3AF]">— reliability: {s.invoice.vendorReliabilityScore}/100</span>
                    </div>
                    <div className="text-right">
                      <div className="font-mono font-bold">{fmt(s.outstanding)}</div>
                      <div className="text-[10px] text-[#9CA3AF]">{s.invoice.daysOverdue}d overdue</div>
                    </div>
                  </div>
                  <div className="bg-black/20 rounded-lg p-3 text-xs mb-2">
                    <div className="font-medium text-amber-600 mb-1">{s.strategy.action}</div>
                    <div className="text-[10px] text-[#6B7280]">{s.strategy.reasoning}</div>
                    <div className="text-[10px] text-[#9CA3AF] mt-1">Channel: {s.strategy.channel} — Follow-up: {s.strategy.followUp}</div>
                  </div>
                  {s.strategy.discountOffer && <div className="text-[10px] text-emerald-600 mb-2">{s.strategy.discountOffer.offer}</div>}
                  <button onClick={() => setExpandedStrategy(expandedStrategy === s.invoice.id ? null : s.invoice.id)} className="text-[10px] text-blue-600">{expandedStrategy === s.invoice.id ? 'Hide' : 'View'} Draft Message</button>
                  {expandedStrategy === s.invoice.id && <pre className="mt-2 bg-black/30 rounded-lg p-3 text-[11px] text-[#4B5563] whitespace-pre-wrap font-sans">{s.strategy.template}</pre>}
                </div>
              ))}
            </>
          )}
        ...
        <Modal wide>
          {healthLoading ? (
            ...
            <>
              ...
              <h4 className="text-xs font-semibold uppercase text-[#9CA3AF] mb-2">Action Checklist</h4>
              {health.checklist?.map((item: any, i: number) => (
                <div key={i} className="flex gap-3 py-2 border-b border-white/[0.03] last:border-0">
                  <span className={'text-[10px] px-2 py-0.5 rounded flex-shrink-0 ' + (item.priority === 'critical' ? 'bg-rose-500/10 text-rose-400' : item.priority === 'high' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600')}>{item.priority}</span>
                  <div><div className="text-xs font-medium">{item.action}</div><div className="text-[10px] text-[#9CA3AF]">{item.detail}</div></div>
                </div>
              ))}

              {vendorDiscounts?.opportunities?.length > 0 && (
                <div className="mt-4">
                  <h4 className="text-xs font-semibold uppercase text-[#9CA3AF] mb-2">Vendor Discount Opportunities — Save {fmt(vendorDiscounts.totalPotentialSavings)}/mo</h4>
                  {vendorDiscounts.opportunities.map((o: any, i: number) => (
                    <div key={i} className="flex justify-between text-xs py-1.5 border-b border-white/[0.03]">
                      <div><span className="font-medium">{o.vendor}</span><span className="text-[#9CA3AF] ml-2">{o.terms}</span></div>
                      <span className="text-emerald-600 font-mono">Save {fmt(o.potentialSavings)}</span>
                    </div>
                  ))}
                </div>
              )}
