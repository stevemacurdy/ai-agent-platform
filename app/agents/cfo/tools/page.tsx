  ...
  return (
    <div className="max-w-[1100px] mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Odoo ERP integration, document scanning, and write-back controls</p>
      </div>
      ...
      {tab === 'odoo' && (
        <div className="bg-[#0A0E15] border border-white/5 rounded-xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-[10px] text-gray-500 uppercase block mb-1">Odoo URL</label>
              <input value={odoo.url} onChange={e => setOdoo({...odoo, url: e.target.value})} placeholder="https://mycompany.odoo.com" className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-sm" /></div>
            <div><label className="text-[10px] text-gray-500 uppercase block mb-1">Database</label>
              <input value={odoo.db} onChange={e => setOdoo({...odoo, db: e.target.value})} placeholder="mycompany-prod" className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-sm" /></div>
            <div><label className="text-[10px] text-gray-500 uppercase block mb-1">Username</label>
              <input value={odoo.username} onChange={e => setOdoo({...odoo, username: e.target.value})} placeholder="admin@company.com" className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-sm" /></div>
            <div><label className="text-[10px] text-gray-500 uppercase block mb-1">API Key</label>
              <input value={odoo.apiKey} onChange={e => setOdoo({...odoo, apiKey: e.target.value})} type="password" placeholder="••••••••" className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-sm" /></div>
          </div>
          <div className="flex gap-3">
            <button onClick={() => show('Connection test: Success! Odoo v17 detected.')} className="px-5 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium">Test Connection</button>
            <button onClick={() => show('Settings saved')} className="px-5 py-2.5 bg-white/5 border border-white/10 text-white rounded-lg text-sm">Save</button>
          </div>
          <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-lg p-3 mt-2">
            <div className="text-xs text-emerald-400 font-medium">Supported Operations</div>
            <div className="text-xs text-gray-500 mt-1">Create/update invoices · Register payments · Post journal entries · Sync vendor bills · Read chart of accounts · Pull financial reports</div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="bg-[#0A0E15] border border-white/5 rounded-xl p-6">
            ...
            <div className="border-2 border-dashed border-white/10 rounded-xl p-8 text-center hover:border-blue-500/30 transition-colors">
              ...
              <label htmlFor="scan-upload" className="cursor-pointer">
                ...
                <div className="text-sm text-gray-400">{scanFile ? scanFile.name : 'Drop a PDF, PNG, or JPG here'}</div>
                <div className="text-xs text-blue-400 mt-2">Browse Files</div>
              </label>
            </div>
            <div className="flex gap-3 mt-4">
              <button onClick={handleScan} disabled={!scanFile || scanning} className="px-5 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium disabled:opacity-50">
                {scanning ? 'Scanning...' : 'Extract Data'}
              </button>
              <label className="px-5 py-2.5 bg-white/5 border border-white/10 text-white rounded-lg text-sm cursor-pointer text-center">
                📷 Camera
          ...
          {scanResult && (
            <div className="bg-[#0A0E15] border border-emerald-500/20 rounded-xl p-6">
              <h3 className="text-sm font-semibold text-emerald-400 mb-3">Extraction Results</h3>
              <div className="grid grid-cols-2 gap-4">
                {Object.entries(scanResult).map(([k, v]) => (
                  <div key={k}><div className="text-[10px] text-gray-500 uppercase">{k}</div><div className="text-sm font-mono mt-0.5">{String(v)}</div></div>
                ))}
              </div>
      ...
      {tab === 'writebacks' && (
        <div className="bg-[#0A0E15] border border-white/5 rounded-xl overflow-hidden">
          <div className="p-4 border-b border-white/5 flex justify-between items-center">
            ...
            <button onClick={() => show('Retrying failed write-backs...')} className="text-xs text-blue-400">Retry Failed</button>
          </div>
          {WRITEBACKS.map(wb => (
            <div key={wb.id} className="flex items-center gap-4 p-4 border-b border-white/[0.03] last:border-0">
              ...
              <div className="flex-1">
                <div className="text-sm">{wb.action} → <span className="font-mono text-blue-400">{wb.target}</span></div>
                <div className="text-[10px] text-gray-600">Odoo: {wb.odooId} · {wb.time}</div>
              </div>
              <span className={"text-[10px] px-2 py-0.5 rounded font-medium " +
                (wb.status === 'synced' ? 'bg-emerald-500/10 text-emerald-400' :
                 wb.status === 'pending' ? 'bg-amber-500/10 text-amber-400' : 'bg-rose-500/10 text-rose-400')}>
                {wb.status}
              </span>
            </div>
          ))}
