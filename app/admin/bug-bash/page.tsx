...
export default function BugBashPage() {
  ...
  const sevColors: Record<string, string> = { high: 'text-rose-400 bg-rose-500/10', medium: 'text-amber-600 bg-amber-50', low: 'text-blue-600 bg-blue-50' }
  ...
  return (
    <div className="max-w-[1100px] mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-[#9CA3AF] mt-1">Track and squash bugs from walkthrough sessions</p>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white border border-[#E5E7EB] rounded-xl p-4"><div className="text-[9px] text-[#9CA3AF] uppercase">Total</div><div className="text-2xl font-mono font-bold mt-1">{bugs.length}</div></div>
        <div className="bg-white border border-[#E5E7EB] rounded-xl p-4"><div className="text-[9px] text-[#9CA3AF] uppercase">Open</div><div className="text-2xl font-mono font-bold text-rose-400 mt-1">{bugs.filter(b => b.status === 'open').length}</div></div>
        <div className="bg-white border border-[#E5E7EB] rounded-xl p-4"><div className="text-[9px] text-[#9CA3AF] uppercase">Fixed</div><div className="text-2xl font-mono font-bold text-emerald-600 mt-1">{bugs.filter(b => b.status === 'fixed').length}</div></div>
      </div>
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-xl p-5">
        ...
        <div className="flex gap-3">
          <input value={newBug.title} onChange={e => setNewBug({...newBug, title: e.target.value})} placeholder="Describe the bug..." className="flex-1 px-3 py-2.5 bg-white border border-[#E5E7EB] shadow-sm rounded-lg text-sm" />
          <select value={newBug.severity} onChange={e => setNewBug({...newBug, severity: e.target.value})} className="px-3 py-2.5 bg-white border border-[#E5E7EB] shadow-sm rounded-lg text-sm">
            <option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option>
          </select>
          <input value={newBug.notes} onChange={e => setNewBug({...newBug, notes: e.target.value})} placeholder="Notes..." className="w-48 px-3 py-2.5 bg-white border border-[#E5E7EB] shadow-sm rounded-lg text-sm" />
          <button onClick={addBug} className="px-5 py-2.5 bg-[#1B2A4A] text-white rounded-lg text-sm font-medium">Report</button>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-2">
        {(['all', 'open', 'fixed'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)} className={"px-4 py-2 rounded-lg text-sm font-medium transition-all " + (filter === f ? 'bg-gray-100 text-white' : 'text-[#9CA3AF] hover:text-[#4B5563]')}>
        ...
        {filtered.map(bug => (
          <div key={bug.id} className="flex items-center gap-4 p-4 border-b border-white/[0.03] last:border-0 hover:bg-white/[0.01]">
            ...
            <div className="flex-1">
              <div className={"text-sm font-medium " + (bug.status === 'fixed' ? 'text-[#6B7280] line-through' : '')}>{bug.title}</div>
              <div className="text-[10px] text-[#6B7280] mt-0.5">{bug.id} · {bug.reporter} · {bug.date}{bug.notes ? ' · ' + bug.notes : ''}</div>
            </div>
