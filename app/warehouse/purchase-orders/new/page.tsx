  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-white/40 mt-1">Create a purchase order for inbound inventory</p>
      </div>
      <div className="space-y-6">
        {/* Vendor & Buyer */}
        <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
          <h2 className="text-sm font-semibold text-white/80 uppercase tracking-wider">Vendor & Buyer</h2>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="block text-xs text-white/50 mb-1">Vendor Name *</label>
            ...
            <div>
              <label className="block text-xs text-white/50 mb-1">Buyer Name</label>
              <input type="text" value={buyerName} onChange={(e) => setBuyerName(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500" />
            </div>
            <div>
              <label className="block text-xs text-white/50 mb-1">Buyer Email</label>
        ...
        {/* Terms */}
        <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
          <h2 className="text-sm font-semibold text-white/80 uppercase tracking-wider">Terms & Shipping</h2>
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-xs text-white/50 mb-1">Payment Terms</label>
            ...
            <div>
              <label className="block text-xs text-white/50 mb-1">Ship Via</label>
              <input type="text" value={shipVia} onChange={(e) => setShipVia(e.target.value)} placeholder="e.g. FedEx Freight"
                className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500" />
            </div>
            <div>
              <label className="block text-xs text-white/50 mb-1">FOB</label>
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
          ...
          <div className="flex items-end gap-2 p-3 bg-white/5 rounded-lg">
            <div className="w-28">
              ...
              <input type="text" value={addSku} onChange={(e) => setAddSku(e.target.value)} placeholder="Optional"
                className="w-full bg-white/5 border border-white/10 rounded-lg px-2 py-2 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500" />
            </div>
            ...
            <div className="w-16">
              ...
              <input type="number" min={1} value={addQty} onChange={(e) => setAddQty(parseInt(e.target.value) || 1)}
                className="w-full bg-white/5 border border-white/10 rounded-lg px-2 py-2 text-sm text-white focus:outline-none focus:border-blue-500" />
            </div>
            <div className="w-24">
            ...
            <div className="w-20">
              <label className="block text-xs text-white/50 mb-1">UOM</label>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="text-left py-2 text-xs text-white/40">#</th>
                  <th className="text-left py-2 text-xs text-white/40">SKU</th>
                  <th className="text-left py-2 text-xs text-white/40">Description</th>
                  <th className="text-right py-2 text-xs text-white/40">Qty</th>
                  <th className="text-center py-2 text-xs text-white/40">UOM</th>
                  <th className="text-right py-2 text-xs text-white/40">Unit Price</th>
                  <th className="text-right py-2 text-xs text-white/40">Total</th>
                  <th></th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {lines.map((l, i) => (
                  <tr key={i}>
                    <td className="py-2 text-xs text-white/30">{i + 1}</td>
                    <td className="py-2 font-mono text-xs text-blue-400">{l.sku || '\u2014'}</td>
                    <td className="py-2 text-white/70">{l.description}</td>
                    <td className="py-2 text-right">
                      <input type="number" min={1} value={l.quantity} onChange={(e) => updateLine(i, 'quantity', parseInt(e.target.value) || 1)}
                        className="w-16 bg-white/5 border border-white/10 rounded px-2 py-1 text-sm text-white text-right focus:outline-none focus:border-blue-500" />
                    </td>
                    <td className="py-2 text-center text-xs text-white/50">{l.unit_of_measure}</td>
                    <td className="py-2 text-right">
                      <input type="number" min={0} step={0.01} value={l.unit_price} onChange={(e) => updateLine(i, 'unit_price', parseFloat(e.target.value) || 0)}
                        className="w-24 bg-white/5 border border-white/10 rounded px-2 py-1 text-sm text-white text-right focus:outline-none focus:border-blue-500" />
                    </td>
              ...
              <tfoot>
                <tr className="border-t border-white/10">
                  <td colSpan={6} className="py-3 text-right text-sm text-white/50">Subtotal</td>
        ...
        <div className="flex items-center gap-3 justify-end">
          ...
          <button onClick={() => submit(true)} disabled={saving || lines.length === 0 || !vendorName.trim()}
            className="px-5 py-2.5 rounded-lg bg-white/10 hover:bg-white/15 disabled:opacity-30 text-sm text-white font-medium transition-colors">Save Draft</button>
