...
const TEMP_BADGES: Record<string, string> = {
  ambient: 'bg-gray-600/30 text-gray-300',
  refrigerated: 'bg-blue-600/30 text-blue-300',
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left px-4 py-3 text-xs text-white/40 font-medium uppercase tracking-wider">SKU</th>
                <th className="text-left px-4 py-3 text-xs text-white/40 font-medium uppercase tracking-wider">Product</th>
                <th className="text-left px-4 py-3 text-xs text-white/40 font-medium uppercase tracking-wider">Customer</th>
                <th className="text-left px-4 py-3 text-xs text-white/40 font-medium uppercase tracking-wider">Location</th>
                <th className="text-right px-4 py-3 text-xs text-white/40 font-medium uppercase tracking-wider">On Hand</th>
                <th className="text-right px-4 py-3 text-xs text-white/40 font-medium uppercase tracking-wider">Allocated</th>
                <th className="text-right px-4 py-3 text-xs text-white/40 font-medium uppercase tracking-wider">Available</th>
                <th className="text-center px-4 py-3 text-xs text-white/40 font-medium uppercase tracking-wider">UOM</th>
                <th className="text-center px-4 py-3 text-xs text-white/40 font-medium uppercase tracking-wider">Zone</th>
              </tr>
            </thead>
                ...
                items.map(item => (
                  <tr key={item.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {item.lot_number && (
                        <span className="ml-2 text-[10px] text-white/30">Lot: {item.lot_number}</span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className="font-mono text-xs text-white/50">{item.location_code || '—'}</span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-center text-xs text-white/50">{item.unit_of_measure}</td>
