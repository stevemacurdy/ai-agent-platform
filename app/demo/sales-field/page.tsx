...
import { 
  ...
  FolderOpen, Bot, Wand2, Share2, Upload
} from 'lucide-react'
...
4. Mention 24/7 support as differentiator
  ...
  const tabs = [
    ...
    { id: 'docs', label: 'Knowledge Base', icon: FolderOpen },
    { id: 'settings', label: 'Settings', icon: Settings },
  ]

  const salesDocs = [
    { id: 1, name: 'Woulf Sales Materials', type: 'PDF', pages: 132, size: '10.2 MB', url: '/docs/woulf-sales-materials.pdf', category: 'Sales Playbook', uploaded: '2026-02-10' },
    { id: 2, name: 'Objection Handling Guide', type: 'PDF', pages: 28, size: '1.8 MB', url: '#', category: 'Training', uploaded: '2026-01-20' },
    { id: 3, name: 'Pricing & Discounting Policy', type: 'PDF', pages: 12, size: '0.8 MB', url: '#', category: 'Policies', uploaded: '2026-01-15' },
    { id: 4, name: 'Competitor Battle Cards', type: 'PDF', pages: 24, size: '2.2 MB', url: '#', category: 'Competitive Intel', uploaded: '2026-01-10' },
    { id: 5, name: 'ROI Calculator Guide', type: 'PDF', pages: 8, size: '0.5 MB', url: '#', category: 'Tools', uploaded: '2025-12-28' },
  ]
    ...
    <div className="min-h-screen bg-[#F4F5F7] text-[#1B2A4A]">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setShowModal('addLead')} className="flex items-center gap-2 bg-[#F5920B] text-white px-4 py-2 rounded-lg font-medium">
      ...
      <div className="border-b border-[#E5E7EB]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex gap-1 overflow-x-auto">
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-3 font-medium transition-colors whitespace-nowrap border-b-2 ${activeTab === tab.id ? 'text-[#1B2A4A] border-[#2A9D8F] font-semibold' : 'text-[#6B7280] border-transparent hover:text-[#1B2A4A]'}`}>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="p-6 bg-white shadow-sm border border-[#E5E7EB] rounded-xl">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => setActiveTab('calendar')} className="text-sm text-[#2A9D8F] hover:text-[#238577]">View All →</button>
                </div>
                    ...
                    <div key={meeting.id} className="flex items-center justify-between p-3 bg-[#F4F5F7] rounded-lg hover:bg-[#e0850a] cursor-pointer" onClick={() => { setSelectedLead(leadsData.find(l => l.name === meeting.contact)); setShowModal('meetingDetail'); }}>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-[#6B7280]">{meeting.contact}</div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm">{meeting.time}</div>
                        <div className="text-xs text-[#6B7280]">{meeting.date}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="p-6 bg-white shadow-sm border border-[#E5E7EB] rounded-xl">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => setActiveTab('leads')} className="text-sm text-[#2A9D8F] hover:text-[#238577]">View All →</button>
                </div>
                <div className="space-y-3">
                  {leadsData.filter(l => l.score >= 80).map((lead) => (
                    <div key={lead.id} className="flex items-center justify-between p-3 bg-[#F4F5F7] rounded-lg hover:bg-[#e0850a] cursor-pointer" onClick={() => { setSelectedLead(lead); setShowModal('leadDetail'); }}>
                      <div className="flex items-center gap-3">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search leads..." className="pl-10 pr-4 py-2 bg-[#F4F5F7] border border-[#E5E7EB] rounded-lg focus:border-[#2A9D8F] focus:outline-none w-64" />
                </div>
            ...
            <div className="bg-white shadow-sm border border-[#E5E7EB] rounded-xl overflow-hidden">
              <table className="w-full">
                ...
                <tbody>
                  {leadsData.map((lead) => (
                    <tr key={lead.id} className="border-b border-[#E5E7EB] hover:bg-white shadow-sm">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-medium">{lead.name}</div>
                            <div className="text-xs text-[#6B7280]">{lead.title}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => handlePreCallResearch(lead)} className="p-2 hover:bg-[#e0850a] rounded-lg" title="Research">
                            <FileText className="w-4 h-4" />
                          </button>
                          <button onClick={() => showNotification(`Calling ${lead.name}...`)} className="p-2 hover:bg-[#e0850a] rounded-lg" title="Call">
                            <Phone className="w-4 h-4" />
                          </button>
                          <button onClick={() => showNotification('Email drafted')} className="p-2 hover:bg-[#e0850a] rounded-lg" title="Email">
                            <Mail className="w-4 h-4" />
                          </button>
                          <button onClick={() => { setSelectedLead(lead); setShowModal('leadDetail'); }} className="p-2 hover:bg-[#e0850a] rounded-lg" title="View">
              ...
              <div className="flex items-end justify-between gap-4 h-48">
                {pipelineStages.map((stage, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center">
                    <div 
                      className="w-full bg-gradient-to-t from-emerald-500/20 to-teal-500/20 rounded-t-lg border border-[#E5E7EB] cursor-pointer hover:from-emerald-500/30 hover:to-teal-500/30 transition-colors"
                    ...
                    />
                    <div className="mt-2 text-xs text-[#6B7280] text-center">{stage.name}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === 'calendar' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowModal('scheduleMeeting')} className="flex items-center gap-2 bg-[#F5920B] text-white px-4 py-2 rounded-lg font-medium">
                  ...
                  {meetingsData.map((meeting) => (
                    <div key={meeting.id} className="flex items-center justify-between p-4 bg-[#F4F5F7] rounded-lg hover:bg-[#e0850a] cursor-pointer" onClick={() => setShowModal('meetingDetail')}>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-medium">{meeting.title}</div>
                          <div className="text-sm text-[#6B7280]">{meeting.contact}</div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-medium">{meeting.time}</div>
                        <div className="text-sm text-[#6B7280]">{meeting.date}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {activeTab === 'docs' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold">Sales Knowledge Base</h2>
              <div className="flex items-center gap-3">
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#6B7280]" />
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#6B7280] mb-3">
                    These documents are used by the AI Coaching feature to provide real-time sales guidance. The Woulf Sales Materials (132 pages) contains product details, pricing, and objection handling scripts that the AI references during calls.
                  </p>
                  <button onClick={() => showNotification('Training AI on documents...')} className="text-sm text-[#2A9D8F] hover:text-[#238577] flex items-center gap-1">
                    <Wand2 className="w-4 h-4" /> Update AI coaching knowledge
                  </button>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'settings' && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold">Sales Employee Settings</h2>
            <div className="grid md:grid-cols-2 gap-6">
              <div className="p-6 bg-white shadow-sm border border-[#E5E7EB] rounded-xl">
                ...
                {researching ? (
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-[#F4F5F7] rounded-lg">
                      <pre className="whitespace-pre-wrap text-sm text-[#4B5563] font-sans">{researchResult}</pre>
                    </div>
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => showNotification('Opening call...')} className="flex-1 py-2 bg-gray-100 rounded-lg font-medium flex items-center justify-center gap-2">
            ...
            {showModal === 'coaching' && (
              <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-[#F4F5F7] rounded-lg">
                    <div className="text-sm text-[#6B7280] mb-2">Suggested opening:</div>
                  ...
                  <div className="p-4 bg-[#F4F5F7] rounded-lg">
                    <div className="text-sm text-[#6B7280] mb-2">Key talking points:</div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" placeholder="John" className="w-full px-4 py-3 bg-[#F4F5F7] border border-[#E5E7EB] rounded-lg" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-2">Last Name</label>
                      <input type="text" placeholder="Smith" className="w-full px-4 py-3 bg-[#F4F5F7] border border-[#E5E7EB] rounded-lg" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-2">Company</label>
                    <input type="text" placeholder="Acme Corp" className="w-full px-4 py-3 bg-[#F4F5F7] border border-[#E5E7EB] rounded-lg" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium mb-2">Email</label>
                      <input type="email" placeholder="john@acme.com" className="w-full px-4 py-3 bg-[#F4F5F7] border border-[#E5E7EB] rounded-lg" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-2">Phone</label>
                      <input type="tel" placeholder="(555) 123-4567" className="w-full px-4 py-3 bg-[#F4F5F7] border border-[#E5E7EB] rounded-lg" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-2">Deal Value</label>
                    <input type="text" placeholder="$50,000" className="w-full px-4 py-3 bg-[#F4F5F7] border border-[#E5E7EB] rounded-lg" />
                  </div>
                  <button onClick={() => { showNotification('Lead added successfully'); setShowModal(null); }} className="w-full py-3 bg-[#F5920B] text-white rounded-lg font-medium">Add Lead</button>
                </div>
              </div>
            )}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <button className="p-3 bg-[#F4F5F7] rounded-lg">Video</button>
                      <button className="p-3 bg-[#F4F5F7] rounded-lg">In-Person</button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium mb-2">Date</label>
                      <input type="date" className="w-full px-4 py-3 bg-[#F4F5F7] border border-[#E5E7EB] rounded-lg" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-2">Time</label>
                      <input type="time" className="w-full px-4 py-3 bg-[#F4F5F7] border border-[#E5E7EB] rounded-lg" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-2">Meeting Title</label>
                    <input type="text" placeholder="Discovery Call" className="w-full px-4 py-3 bg-[#F4F5F7] border border-[#E5E7EB] rounded-lg" />
                  </div>
                  <button onClick={() => { showNotification('Meeting scheduled'); setShowModal(null); }} className="w-full py-3 bg-[#F5920B] text-white rounded-lg font-medium">Schedule Meeting</button>
                </div>
              </div>
            )}
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-[#6B7280]">{selectedLead.title}</div>
                      <div className="text-sm text-[#9CA3AF]">{selectedLead.company}</div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => showNotification('Email drafted')} className="flex-1 py-2 bg-gray-100 rounded-lg flex items-center justify-center gap-2">
                      <Mail className="w-4 h-4" /> Email
                    </button>
                    <button onClick={() => { setShowModal(null); handlePreCallResearch(selectedLead); }} className="flex-1 py-2 bg-gray-100 rounded-lg flex items-center justify-center gap-2">
