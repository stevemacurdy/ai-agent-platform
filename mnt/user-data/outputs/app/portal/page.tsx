...
/* ═══════════════════════════════════════════════════════════════
   WoulfAI Portal Dashboard — Branded Design System
...
export default function PortalPage() {
  ...
  return (
    <div
      ...
      style={{ background: '#F4F5F7', color: '#1A1A2E', fontFamily: "'DM Sans', -apple-system, sans-serif" }}
    >
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800&family=Outfit:wght@400;500;600;700;800;900&display=swap');
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between h-[60px]">
          ...
          <div className="flex items-center gap-4">
            ...
            <div
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl cursor-pointer hover:bg-white/[0.06] transition-all"
      ...
      <main className="max-w-7xl mx-auto px-6 sm:px-8 py-8">
        ...
        {welcome && (
          <div
            className="mb-6 px-5 py-4 rounded-2xl flex items-center gap-3 fade-up"
            style={{
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8 fade-up">
          ...
          <div className="flex items-center gap-3">
            ...
            {user?.role === 'admin' && (
              <Link
                href="/admin"
                className="px-4 py-2.5 rounded-xl text-[13px] font-semibold transition-all hover:-translate-y-px"
                style={{
                  background: '#1B2A4A',
                  color: '#fff',
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8 fade-up fade-up-1">
          {[
            ...
            <div
              key={stat.label}
              className="p-5 rounded-2xl bg-white border transition-all hover:-translate-y-px hover:shadow-lg"
              style={{
                borderColor: '#E5E7EB',
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span
                  ...
                  style={{ background: stat.accent, animation: 'pulse-dot 2s infinite' }}
                />
              </div>
              <p
            ...
            <div className="col-span-full text-center py-16">
              ...
              <p className="text-[14px] text-gray-500 mt-1.5">
                {searchQuery
                  ...
                  : 'Contact your admin to enable AI Employees for your workspace'}
              </p>
            </div>
          ) : (
            filteredAgents.map((agent) => {
              const catColor = CATEGORY_COLORS[agent.category || ''] || CATEGORY_COLORS.default;
              const icon = agent.icon || DEFAULT_ICONS[agent.slug] || '🤖';

              return (
                <Link
                  key={agent.id || agent.slug}
                  href={`/portal/agent/${agent.slug || agent.id}`}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span
                          ...
                          style={{
                            background: agent.status === 'inactive' ? '#E5E7EB' : '#2A9D8F',
                            animation: agent.status !== 'inactive' ? 'pulse-dot 2s infinite' : 'none',
                          }}
                        />
                      </div>
                      {agent.category && (
                        <span
                          className="inline-block mt-1 text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider"
                      ...
                      {agent.description && (
                        <p className="text-[12px] text-gray-500 mt-2 leading-relaxed line-clamp-2">
                          {agent.description}
                        </p>
                      )}
                    </div>
        ...
        <div className="mt-10 grid sm:grid-cols-3 gap-4 fade-up fade-up-3">
          {[
            ...
            >
              ...
              <div>
                <p className="font-bold text-[14px]" style={{ color: '#1B2A4A' }}>
                  {link.label}
                </p>
      ...
      <footer className="mt-12 py-6 px-6 sm:px-8 border-t" style={{ borderColor: '#E5E7EB' }}>
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-4">
            <Link href="/security" className="text-[11px] text-gray-400 hover:text-gray-600 transition-colors">
              Security
            </Link>
            <Link href="/terms" className="text-[11px] text-gray-400 hover:text-gray-600 transition-colors">
              Terms
            </Link>
            <Link href="/privacy" className="text-[11px] text-gray-400 hover:text-gray-600 transition-colors">
              Privacy
            </Link>
          </div>
        </div>
